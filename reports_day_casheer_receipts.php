...
mysql_connect("$host", "$user", "$pass")or die("cannot connect");
mysql_select_db("$db")or die("cannot select DB");		
$query = "SELECT  SUM(money) FROM reports WHERE day = $rday AND month = $rmonth AND year = $ryear AND `casheer` = '$Rcash'"; 
...
$result = mysql_query("SELECT *,SUM(money) FROM `reports` WHERE day = $rday AND month = $rmonth AND year = $ryear AND `casheer` = '$Rcash' AND End_hour != '-' GROUP BY session_id"); 
 ?><thead>
<tr>
